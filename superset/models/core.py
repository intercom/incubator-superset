...
string_cleanser = config["STRING_CLEANSER"]
...
class Database(
    ...
    def get_df(  # pylint: disable=too-many-locals
        ...
        def needs_string_cleansing(df_series: pd.Series) -> bool:
        ...
        with closing(engine.raw_connection()) as conn:
            with closing(conn.cursor()) as cursor:
                ...
                for k, v in df.dtypes.items():
                    ...
                    if v.type == numpy.object_ and needs_string_cleansing(df[k]):
                            df[k] = df[k].apply(string_cleanser)
                return df
